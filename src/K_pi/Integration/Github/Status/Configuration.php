...
final class Configuration
{
     ...
     */
    public function __construct(mixed $configuration, public readonly string $reportName)
    {
        ...
        if (false === is_object($configuration)) {
            throw new AtPathException(
                sprintf('.reports.%s.check-reporter.%s', $this->reportName, CheckReporterIntegration::GITHUB_STATUS->value),
                'must be null or an object',
            );
        }

        [$this->singularUnit, $this->pluralUnit] = $this->getUnits($configuration);
    ...
    private function getUnits(object $configuration): array
    {
        ...
        if (false === property_exists($unit, 'singular') || false === property_exists($unit, 'plural')) {
            throw new AtPathException(
                sprintf('.reports.%s.check-reporter.%s.unit', $this->reportName, CheckReporterIntegration::GITHUB_STATUS->value),
        ...
        if (false === is_string($singular)) {
            throw new AtPathException(
                sprintf('.reports.%s.check-reporter.%s.unit.singular', $this->reportName, CheckReporterIntegration::GITHUB_STATUS->value),
                'must be a string',
            );
        }
        ...
        if (false === is_string($plural)) {
            throw new AtPathException(
                sprintf('.reports.%s.check-reporter.%s.unit.plural', $this->reportName, CheckReporterIntegration::GITHUB_STATUS->value),
                'must be a string',
            );
        }
    ...
    private function getStates(object $configuration): array
    {
        ...
        $path = sprintf('.reports.%s.check-reporter.%s.states', $this->reportName, CheckReporterIntegration::GITHUB_STATUS->value);
        $message = sprintf(
            'must be %s or an object with "on-lower" and "on-higher" properties',
            join(' or ', array_map(fn (string $key) => '"' . $key . '"', array_keys(self::PREBUILD_STATES))),
        );
        ...
        $resolve = function (string $property) use ($states, $path, $message): StatusState {
            ...
            if (false === is_string($state) || null === $enum = StatusState::tryFrom($state)) {
                throw new AtPathException(
                    $path.'.'.$property,
                    sprintf(
                        'must be %s',
                        join(' or ', array_map(fn (StatusState $enum) => '"' . $enum->value . '"', StatusState::cases())),
                    )
                );
            }
